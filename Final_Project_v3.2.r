...
# Assuming your data frame is named OG
...
# add number of instances by cluster from guns_f1
...
## Using guns_f now we will create some analytics graphs


# timeline graph of incidents frequency by cluster
guns_f
View(guns_f)
...
# Create a line plot with a more sober color palette
# Create a line chart with a Set2 color palette
# Create a line chart with a Set2 color palette
...
# break year and month into two columns
df_pivoted$year <- substr(df_pivoted$year_month, 1, 4)
df_pivoted$month <- substr(df_pivoted$year_month, 6, 7)

# Print the pivoted data frame
print(df_pivoted)

# linear regression for cluster 1
# Create a linear regression model for cluster 1
df_pivoted$numeric_date <- as.numeric(df_pivoted$year_month)
df_pivoted$year <- as.numeric(df_pivoted$year)
model_cluster_1 <- lm(X4 ~ year, data = df_pivoted)
# Print the model
print(model_cluster_1)

# Extract intercept and slope from the linear model
intercept <- coef(model_cluster_1)[1]
slope <- coef(model_cluster_1)[2]

# Plot the scatter plot with the regression line
ggplot(df_pivoted, aes(x = year, y = X4)) +
  geom_point(color = "blue") +  # Actual data points
  geom_smooth(method = "lm", se = FALSE, color = "red") +  # Regression line without confidence interval
  labs(title = "Scatter Plot with Linear Regression for Cluster 1",
       x = "Year",
       y = "X1") +
  theme_minimal() +
  geom_abline(intercept = intercept, slope = slope, col = "red", lwd = 1)  # Regression line with confidence interval

## Predict the number of incidents for 2024
new_data <- data.frame(year = as.character(2024))  # Convert the year to character

predicted_value <- predict(model_cluster_1, newdata = new_data)

print(slope*2024 + intercept)
