...
gun <- gun %>%
  select(
    incident_id, date, state, city_or_county, n_killed, n_injured, congressional_district, gun_stolen, gun_type, incident_characteristics,
    n_guns_involved, notes, participant_age_group, participant_gender, 
    participant_relationship, participant_status, participant_type
  ) %>%
  select(-c("incident_url_fields_missing", "location", "address", "sources", "incident_url", "source_url")) %>%
  select(-c("state_house_district", "state_senate_district")) %>%
