...
# Assuming your dataframe is named 'gun'
...
X <- gun[, !(names(gun) %in% c('participant_gender', 'gun_stolen','female_count', 'adult_count', 'stolen_count', 'total_count', 'injured_count', 'killed_count',"incident_id", "date", "notes", "incident_characteristics", "gun_type", "participant_relationship", "location_description"))]
X[is.infinite(X)] <- 0
X[is.na(X)] <- 0
...
OG <-X 
X <- scale(X)
# Replace infinite values with 0 in the entire data frame
