...
# file_path <- "/Users/arham/Downloads/02. MVS/Final Project/Dataset 1 — Gun violence.csv" # nolint
...
# # lower case all the text in the "participant_relationship" column
# gun$participant_relationship <- tolower(gun$participant_relationship)
# # new column relation_family = 1 if the keyword 'family' is present in the "participant_relationship" column
# gun$relation_family <- sapply(gun$participant_relationship, function(x) length(grep("family", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_random_victims = 1 if the keyword 'random victims' is present in the "participant_relationship" column
# gun$relation_random_victims <- sapply(gun$participant_relationship, function(x) length(grep("random victims", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_aquaintance = 1 if the keyword 'aquaintance' is present in the "participant_relationship" column
# gun$relation_aquaintance <- sapply(gun$participant_relationship, function(x) length(grep("aquaintance", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_significant_others = 1 if the keyword 'significant others' is present in the "participant_relationship" column
# gun$relation_significant_others <- sapply(gun$participant_relationship, function(x) length(grep("significant others", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_armed_robbery = 1 if the keyword 'armed robbery' is present in the "participant_relationship" column
# gun$relation_armed_robbery <- sapply(gun$participant_relationship, function(x) length(grep("armed robbery", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_gang = 1 if the keyword 'gang' is present in the "participant_relationship" column
# gun$relation_gang <- sapply(gun$participant_relationship, function(x) length(grep("gang", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_mass_shooting = 1 if the keyword 'mass shooting' is present in the "participant_relationship" column
# gun$relation_mass_shooting <- sapply(gun$participant_relationship, function(x) length(grep("mass shooting", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_knows_victims = 1 if the keyword 'knows victims' is present in the "participant_relationship" column
# gun$relation_knows_victims <- sapply(gun$participant_relationship, function(x) length(grep("knows victims", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_co_worker = 1 if the keyword 'co-worker' is present in the "participant_relationship" column
# gun$relation_co_worker <- sapply(gun$participant_relationship, function(x) length(grep("co-worker", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_neighbor = 1 if the keyword 'neighbor' is present in the "participant_relationship" column
# gun$relation_neighbor <- sapply(gun$participant_relationship, function(x) length(grep("neighbor", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_friends = 1 if the keyword 'friends' is present in the "participant_relationship" column
# gun$relation_friends <- sapply(gun$participant_relationship, function(x) length(grep("friends", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_home_invasion = 1 if the keyword 'home invasion' is present in the "participant_relationship" column
# gun$relation_home_invasion <- sapply(gun$participant_relationship, function(x) length(grep("home invasion", strsplit(x, split = "\\|\\|")[[1]])))
# # new column relation_does_not_know_victim = 1 if the keyword 'does not know victim' is present in the "participant_relationship" column
# gun$relation_does_not_know_victim <- sapply(gun$participant_relationship, function(x) length(grep("does not know victim", strsplit(x, split = "\\|\\|")[[1]])))
# # remove participant_relationship column
# gun <- gun[, !(names(gun) %in% "participant_relationship")]

# # apply pca on relationship columns and keep top 2 components
# gun_pca <- prcomp(gun[, c("relation_family", "relation_random_victims", "relation_aquaintance", "relation_significant_others", "relation_armed_robbery", "relation_gang", "relation_mass_shooting", "relation_knows_victims", "relation_co_worker", "relation_neighbor", "relation_friends", "relation_home_invasion", "relation_does_not_know_victim")], scale = TRUE)
# # add pca columns to gun
# gun$relation_pca1 <- gun_pca$x[, 1]
# gun$relation_pca2 <- gun_pca$x[, 2]
# # remove relationship columns
# gun <- gun[, !(names(gun) %in% c("relation_family", "relation_random_victims", "relation_aquaintance", "relation_significant_others", "relation_armed_robbery", "relation_gang", "relation_mass_shooting", "relation_knows_victims", "relation_co_worker", "relation_neighbor", "relation_friends", "relation_home_invasion", "relation_does_not_know_victim"))]

# autoplot(gun_pca, data = gun, loadings = TRUE, loadings.label = TRUE) +
#   theme_minimal() +
#   theme(legend.position = 'bottom') +
#   ggtitle("Relationship PCA")
...
# # save to csv file titled gun_preprocessing.csv
# write.csv(gun, file = "gun_preprocessing.csv", row.names = FALSE)

# read gun_preprocessing.csv
gun <- read.csv("gun_preprocessing.csv")
...
exclude_columns_X <- c('incident_id', 'date', 'notes', 'incident_characteristics', 'gun_type', 'participant_relationship', 'location_description',
                     'participant_gender','participant_type', 'participant_age','adult_count', 'gun_stolen','unharmed_count','stolen_count',
                      'total_count', 'injured_count', 'killed_count', 'n_guns_involved', 'victim_count',
                      'relation_family', 'relation_random_victims', 'relation_aquaintance', 'relation_significant_others', 'relation_armed_robbery', 'relation_gang',
                      'relation_mass_shooting', 'relation_knows_victims', 'relation_co_worker', 'relation_neighbor', 'relation_friends', 'relation_home_invasion', 'relation_does_not_know_victim'
                    )  # 'n_handguns', 'n_rifle', 'n_shotgun', 'n_other'
X <- gun[, !(names(gun) %in% exclude_columns_X)]
...
# k means clustering
set.seed(123)
kmeans_Z <- kmeans(W, centers = 8, nstart = 25)
kmeans_Z$cluster <- as.factor(kmeans_Z$cluster)
W <- cbind(W, cluster = kmeans_Z$cluster)

# add cluster names to PR by incident id
PR$cluster_kmeans <- W$cluster

table(PR$cluster)

#  'n_handguns', 'n_rifle', 'n_shotgun', 'n_other' summary by cluster
aggregate(PR[, c('n_handguns', 'n_rifle', 'n_shotgun', 'n_other')], by = list(PR$cluster), FUN = mean)


View(W)
pca <- prcomp(W[, -c(0, ncol(W))])
...
# k means clustering
set.seed(123)
kmeans_W <- kmeans(W, centers = 7, nstart = 25)
kmeans_W$cluster <- as.factor(kmeans_W$cluster)
W <- cbind(W, cluster = kmeans_W$cluster)
pca <- prcomp(W[, -c(1, ncol(W))], scale = TRUE)

autoplot(pca, data = W, loadings = TRUE, loadings.label = TRUE) +
  theme_minimal() +
  theme(legend.position = 'bottom') +
  ggtitle("Crime Scene Incident Split")
...
# set.seed(123)
# hc <- hclust(dist(D[, -1]), method = "ward.D2")
...
# # plot clusters
# fviz_cluster(list(data = D[, -1], cluster = clusters), geom = "point", stand = FALSE, ellipse.type = "convex", palette = "jco", ggtheme = theme_minimal())
